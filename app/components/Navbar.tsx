...
const navigationData = [
  { label: "HOME", href: "/" },
  { label: "LOCATIONS", href: "#", dropdownItems: [{ label: "MANHATTAN", href: "/locations/manhattan" }, { label: "BERGEN COUNTY", href: "/locations/bergen" }] },
  { label: "PRODUCTS", href: "#", dropdownItems: [
      ...
      { label: "SOLERA SOFT SHADES", href: "/products/solera-soft" },
      { label: "SONNETTE SHADES", href: "/products/sonnette" },
      { label: "PIROUETTE SHADINGS", href: "/products/pirouette" },
      { label: "LUMINETTE SHEERS", href: "/products/luminette" },
      { label: "WOVEN WOOD SHADES", href: "/products/woven-wood" },
      { label: "WOOD BLINDS", href: "/products/wood-blinds" },
...
export default function Navbar() {
  ...
  return (
    <>
      <div className={`h-[5px] w-full ${GRADIENT_CLASS}`} />
      <header className="w-full bg-white border-b border-gray-100 shadow-sm sticky top-0 z-40">
        <div className="max-w-[1400px] mx-auto px-4 h-24 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-6 text-[13px] tracking-widest text-gray-600 font-bold h-full">
            {navigationData.map((item, index) => (
              <div key={index} className="relative group flex items-center h-full">
                <Link href={item.href} className="hover:text-black py-8 flex items-center gap-1 transition-colors relative group/link">
                  <span className="relative py-1">
                    {item.label}
                    <div className={`absolute bottom-0 left-0 w-full h-[3px] ${GRADIENT_CLASS} opacity-0 group-hover/link:opacity-100 transition-opacity`} />
                  </span>
                  {item.dropdownItems && <ChevronDown size={14} />}
                </Link>
                
                {item.dropdownItems && (
                  <div className="absolute top-[calc(100%-8px)] left-0 w-60 bg-white shadow-xl border-t-2 border-gray-200 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                    {item.dropdownItems.map((sub, sIdx) => (
                      <Link key={sIdx} href={sub.href} className="block px-6 py-3 text-[10px] tracking-widest text-gray-600 hover:bg-gray-50">{sub.label}</Link>
                    ))}
                  </div>
                )}
              </div>
          ...
          <button onClick={() => setIsDrawerOpen(true)} className="lg:hidden p-2"><Menu size={24} /></button>
        </div>
      </header>
      ...
      <aside className={`fixed top-0 right-0 h-full w-[300px] bg-white z-50 transform transition-transform flex flex-col ${isDrawerOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="p-6 border-b flex justify-between items-center shrink-0">
          ...
          <X size={24} onClick={() => setIsDrawerOpen(false)} className="cursor-pointer" />
        </div>
            ...
            {navigationData.map((item, i) => (
                ...
                {item.dropdownItems ? (
                  <div 
                    ...
                    className="flex justify-between items-center py-4 cursor-pointer hover:text-amber-800"
                  >
                  ...
                  <Link 
                    ...
                    className="flex justify-between items-center py-4 cursor-pointer hover:text-amber-800 text-sm font-bold tracking-widest w-full block"
                  >
                    {item.label}
                  </Link>
                )}
                ...
                {item.dropdownItems && openDropdown === item.label && (
                    <div className="bg-gray-50 p-2 flex flex-col gap-1 mb-2">
                    {item.dropdownItems.map((sub, s) => (
                        <Link 
                          key={s} 
                          href={sub.href} 
                          onClick={handleLinkClick}
                          className="text-[11px] py-2 px-4 text-gray-600 hover:text-black uppercase"
                        >
                          {sub.label}
                        </Link>
                    ))}
        ...
        <div className="p-6 border-t flex flex-col gap-3 shrink-0">
